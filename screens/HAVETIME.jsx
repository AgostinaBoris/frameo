// figma node: 180:268 HAVE TIME (2 variants)
import { useLanguage } from '../src/i18n.jsx';
const __venc = (v) => String(v).replace(/[%|=]/g, encodeURIComponent);
const __vkey = (p) => "property1=" + __venc(p.property1);

export function HAVETIME(_p = {}) {
  ...
  const __body0 = () => (
    <div className={`selectable-card ${props.className ?? ''}`} style={{
      ...
      }}>{props.text1 ?? t('time.have.duration')}</span>
      <div style={{
        position: "absolute",
        left: 0,
        top: 0,
      ...
      <div className="fig-asset-4f7d2d04ee3f0906" style={{
        position: "absolute",
        left: 22,
        top: 20,
        width: 80,
      ...
      <span style={{
        ...
        color: "rgb(255,255,255)",
      }}>{props.text2 ?? t('time.have.label')}</span>
    </div>
  );
  const __body1 = () => (
    <div className={`selectable-card ${props.className ?? ''}`} style={{
